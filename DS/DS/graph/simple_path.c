...
int hasSimplePath(struct graph * g  , int s , int d , int *visited)
{
	int t ;
	visited[s] =  1;
	if(s == d)
	...
	for(t = 0 ; t < g -> v ; t++)
	{
		if(!visited[t] && g -> adj[s][t])
			{
				printVisit(g , visited) ;
				if(hasSimplePath(g , t , d , visited))
					return 1 ;
			}
	}
	return 0 ; 
...
int main()
{
	...
	if(hasSimplePath(g , s , d , visited))
		printf("There is a simple path between %d and %d\n", s ,d);
	else
	{
		printf("There is a NO simple path between %d and %d\n", s ,d);
	}
}